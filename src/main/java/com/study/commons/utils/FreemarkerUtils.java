...
public class FreemarkerUtils {
	...
	public static void exportImageExcel(String excelFilePath, FreemarkerInput freemakerEntity) {
		try {
			...
			FileUtils.forceDelete(
					new File(freemakerEntity.getTemporaryXmlfile() + freemakerEntity.getFileName() + ".xml"));
	...
	public static void exportImageExcel(HttpServletResponse response, FreemarkerInput freemakerInput) {
		try {
			...
			// 删除xml缓存文件
			FileUtils.forceDelete(
					new File(freemakerInput.getTemporaryXmlfile() + freemakerInput.getFileName() + ".xml"));
		} catch (Exception e) {
			e.printStackTrace();
		}
